...
class DexYCB(HDataset):
    ...
    def _preload(self):
        ...
        self.cache_identifier_dict = {
            "cache_version": "v2_restore_multiview_8cam",
    ...
    def get_obj_corners(self, idx):
        ...
        obj_id = sample["ycb_ids"][grasp_ind]
        pose_y = label["pose_y"][grasp_ind]
        R, t = pose_y[:3, :3], pose_y[:, 3:]
        
        corners_rest = self.obj_rest_corners[obj_id]  # (8, 3)
        corners_cam = (R @ corners_rest.T).T + t.T  # (8, 3)

        return corners_cam.astype(np.float32)

    def get_obj_pose_and_points(self, idx):
        """Return object pose labels and point clouds in the camera frame."""
        ori_idx = self.sample_idxs[idx]
        sample = self.dex_ycb[ori_idx]
        label = self.get_label(sample["label_file"])
        ...
        R_label = R_obj_cam.astype(np.float32)

        if len(t_obj_cam.shape) == 1:
    ...
    def get_bbox_hand_obj(self, idx, scale_factor=1.25):
        hand_joints_2d = self.get_joints_2d(idx)  # (21, 2)
        obj_corners = self.get_obj_corners(idx)  # (8, 3)
        ori_idx = self.sample_idxs[idx]
        sample = self.dex_ycb[ori_idx]
        ...
        obj_corners_2d = np.zeros((8, 2), dtype=np.float32)
        obj_corners_2d[:, 0] = fx * obj_corners[:, 0] / obj_corners[:, 2] + cx
        obj_corners_2d[:, 1] = fy * obj_corners[:, 1] / obj_corners[:, 2] + cy
...
class DexYCBMultiView(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg):
        ...
        if self.data_split == "test" and not self.test_with_multiview:
            warnings.warn(f"test_with_multiview = {self.test_with_multiview} is deprecated and will be removed soon")
    ...
    def set_stage(self, stage_name: str):
        ...
        for dataset in self.set_mappings.values():
