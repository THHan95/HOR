...
from ..utils.transform import (aa_to_rotmat, batch_cam_extr_transf, batch_cam_intr_projection, batch_persp_project,
                               mano_to_openpose, rot6d_to_rotmat, rotmat_to_rot6d, rotmat_to_aa)
...
class POEM_HeatmapCenterRot(nn.Module, ModuleAbstract):
    def __init__(self, cfg):
        super(POEM_HeatmapCenterRot, self).__init__()
        ...
        self.sv_obj_axisang_abs_max = math.pi
        ...
        self.sv_obj_rot_style = str(cfg.get("SV_OBJECT_ROT_STYLE", "artiboost_axisang")).lower()
        if self.sv_obj_rot_style != "artiboost_axisang":
            logger.warning(
                f"{self.name} only supports artiboost_axisang for single-view object rotation in this branch; "
                f"got {self.sv_obj_rot_style}, forcing axis-angle behavior."
            )
        ...
        self.sv_object_token_adapter = nn.Sequential(
            nn.Linear(self.feat_dim, self.feat_dim),
            nn.ReLU(),
            nn.Linear(self.feat_dim, self.feat_dim),
        )
        self.sv_hand_token_adapter = nn.Sequential(
            nn.Linear(self.feat_dim, self.feat_dim),
            nn.ReLU(),
            nn.Linear(self.feat_dim, self.feat_dim),
        )
        self.sv_object_hand_cross_attn = nn.MultiheadAttention(
            embed_dim=self.feat_dim,
            num_heads=4,
            dropout=0.0,
            batch_first=True,
        )
        self.sv_object_pose_norm = nn.LayerNorm(self.feat_dim)
        self.sv_object_pose_ffn = nn.Sequential(
            nn.Linear(self.feat_dim, self.feat_dim),
            nn.ReLU(),
            nn.Linear(self.feat_dim, self.feat_dim),
        )
    ...
    def _rotation_matrix_from_axis_angle(axis, angle):
        ...
        rotmat, _ = cv2.Rodrigues(rotvec.astype(np.float32))
        return rotmat.astype(np.float32)

    def _resolve_symmetry_model_info_path(self):
        loss_cfg = getattr(self.cfg, "LOSS", None)
        if loss_cfg is not None and loss_cfg.get("MODEL_INFO_PATH", None):
            return loss_cfg.MODEL_INFO_PATH
        ...
        if dataset_cfg is not None:
            for split_name in ["TRAIN", "TEST"]:
                ...
                if split_cfg is not None and split_cfg.get("DATA_ROOT", None):
                    data_roots.append(split_cfg.DATA_ROOT)
    ...
    def _get_symmetry_transformations(self, model_info, max_sym_disc_step):
        trans_disc = [{"R": np.eye(3, dtype=np.float32), "t": np.zeros((3, 1), dtype=np.float32)}]
        if "symmetries_discrete" in model_info:
            for sym in model_info["symmetries_discrete"]:
                ...
                trans_disc.append({"R": sym_4x4[:3, :3], "t": sym_4x4[:3, 3:].reshape(3, 1) / 1000.0})
        ...
        if "symmetries_continuous" in model_info:
            for sym in model_info["symmetries_continuous"]:
                ...
                discrete_steps_count = int(np.ceil(np.pi / max(max_sym_disc_step, 1e-6)))
                discrete_steps_count = max(discrete_steps_count, 1)
                ...
                for idx in range(1, discrete_steps_count):
                    ...
                    trans_cont.append({"R": rot, "t": trans})

        trans = []
        for disc in trans_disc:
            if trans_cont:
                for cont in trans_cont:
                    trans.append({
    ...
    def _init_object_symmetry(self):
        model_info_path = self._resolve_symmetry_model_info_path()
        identity_R = torch.eye(3, dtype=torch.float32).view(1, 1, 3, 3)
        identity_t = torch.zeros(1, 1, 3, 1, dtype=torch.float32)
        self.register_buffer("symmetry_R", identity_R)
        self.register_buffer("symmetry_t", identity_t)
        self.symmetry_enabled = False

        if model_info_path is None or not os.path.isfile(model_info_path):
            logger.warning(f"{self.name} symmetry metadata not found, disabling SymCornerLoss")
            return

        with open(model_info_path, "r") as f_info:
        ...
        for obj_id_str, obj_meta in model_info.items():
            ...
            model_sym[obj_id] = self._get_symmetry_transformations(obj_meta, self.obj_sym_max_disc_step)
            max_sym_len = max(max_sym_len, len(model_sym[obj_id]))

        sym_R = np.tile(np.eye(3, dtype=np.float32)[None, None], (max_obj_id + 1, max_sym_len, 1, 1))
        sym_t = np.zeros((max_obj_id + 1, max_sym_len, 3, 1), dtype=np.float32)
        for obj_id, transforms in model_sym.items():
            for idx, transform in enumerate(transforms):
                sym_R[obj_id, idx] = transform["R"]
                sym_t[obj_id, idx] = transform["t"]
            for idx in range(len(transforms), max_sym_len):
                sym_R[obj_id, idx] = np.eye(3, dtype=np.float32)
                sym_t[obj_id, idx] = np.zeros((3, 1), dtype=np.float32)

        self.symmetry_R = torch.from_numpy(sym_R)
        self.symmetry_t = torch.from_numpy(sym_t)
        self.symmetry_enabled = True
        logger.info(f"{self.name} loaded object symmetry metadata from {model_info_path}")

    @staticmethod
    def _get_master_obj_ids(batch):
        obj_ids = batch.get("obj_id", None)
        if obj_ids is None:
            return None
        if obj_ids.dim() == 1:
            return obj_ids.long()
        return obj_ids[:, 0].long()

    def _symmetry_corner_loss(self, pred_corners_abs, corner_rest, gt_rot6d, gt_center_abs, obj_ids):
        if (
            not self.symmetry_enabled
            or self.obj_sym_corner_weight <= 0
            or pred_corners_abs is None
            or corner_rest is None
            or gt_rot6d is None
            or gt_center_abs is None
            or obj_ids is None
        ):
            return pred_corners_abs.new_tensor(0.0) if pred_corners_abs is not None else torch.tensor(0.0)

        obj_ids = obj_ids.to(device=pred_corners_abs.device, dtype=torch.long)
        obj_ids = obj_ids.clamp_(0, self.symmetry_R.shape[0] - 1)
        sym_R = self.symmetry_R[obj_ids].to(device=pred_corners_abs.device, dtype=pred_corners_abs.dtype)
        sym_t = self.symmetry_t[obj_ids].to(device=pred_corners_abs.device, dtype=pred_corners_abs.dtype)
        corner_rest = corner_rest.to(device=pred_corners_abs.device, dtype=pred_corners_abs.dtype)
        gt_rotmat = rot6d_to_rotmat(gt_rot6d.to(device=pred_corners_abs.device, dtype=pred_corners_abs.dtype))
        gt_center_abs = gt_center_abs.to(device=pred_corners_abs.device, dtype=pred_corners_abs.dtype)

        sym_corner_can = torch.einsum("bsij,bnj->bsni", sym_R, corner_rest) + sym_t.squeeze(-1).unsqueeze(-2)
        sym_corner_abs = torch.einsum("bij,bsnj->bsni", gt_rotmat, sym_corner_can) + gt_center_abs[:, None, None, :]
        corner_errors = ((sym_corner_abs - pred_corners_abs[:, None, :, :]) ** 2).mean(dim=-1).mean(dim=-1)
        return corner_errors.min(dim=1)[0].mean()

    def _compute_stage1_object_geometry_losses(
        self,
        preds,
        gt,
        pred_ref_hand_root_views,
        master_hand_joints_gt,
        ...
        if self.obj_chamfer_weight <= 0 and self.obj_emd_weight <= 0 and self.obj_sym_corner_weight <= 0:
        ...
        target_joints_3d_no_rot = gt.get("target_joints_3d_no_rot", None)
        obj_ids = self._get_master_obj_ids(gt)
        ...
        if (
            ...
            and pred_ref_hand is not None
            and master_obj_sparse_rest is not None
        ):
            pred_master_points = self._build_object_points_from_hand_pose(
                master_obj_sparse_rest,
                pred_obj_init_rot6d,
                pred_ref_hand[:, self.center_idx:self.center_idx + 1],
                pred_obj_init_trans,
            )
            if self.obj_chamfer_weight > 0:
                loss_chamfer_raw, _ = self.chamfer_loss(pred_master_points.float(), master_obj_sparse_gt.float())
                loss_chamfer_terms.append(loss_chamfer_raw)
            if self.obj_emd_weight > 0:
                loss_emd_raw = torch.mean(
                    self.earth_mover_loss(pred_master_points.float(), master_obj_sparse_gt.float(), transpose=False)
                    / pred_master_points.shape[1]
                )
                loss_emd_terms.append(loss_emd_raw)
            if master_obj_kp21_rest is not None and self.obj_sym_corner_weight > 0:
                pred_master_center_abs = pred_ref_hand[:, self.center_idx] + pred_obj_init_trans
                pred_master_corners = self._build_object_points_from_pose(
                    master_obj_kp21_rest[:, :8],
                    pred_obj_init_rot6d,
                    pred_master_center_abs,
                )
                gt_master_center_abs = master_hand_joints_gt[:, self.center_idx] + master_obj_trans_gt
                loss_sym_terms.append(
                    self._symmetry_corner_loss(
                        pred_master_corners,
                        master_obj_kp21_rest[:, :8],
                        master_obj_rot6d_gt,
                        gt_master_center_abs,
                        obj_ids,
                    )
                )
        ...
        if (
            pred_obj_view_rot6d_cam is not None
            and pred_obj_view_trans is not None
            and master_obj_sparse_rest is not None
            and target_obj_pc_sparse is not None
        ):
            pred_sv_points = self._build_object_points_from_hand_pose(
                master_obj_sparse_rest,
                pred_obj_view_rot6d_cam,
                pred_ref_hand_root_views,
                pred_obj_view_trans,
            )
            pred_sv_points = pred_sv_points.view_as(target_obj_pc_sparse)
            if self.obj_chamfer_weight > 0:
                loss_chamfer_raw, _ = self.chamfer_loss(
                    pred_sv_points.flatten(0, 1).float(),
                    target_obj_pc_sparse.flatten(0, 1).float(),
                )
                loss_chamfer_terms.append(loss_chamfer_raw)
            if self.obj_emd_weight > 0:
                pred_sv_points_flat = pred_sv_points.flatten(0, 1)
                target_obj_pc_sparse_flat = target_obj_pc_sparse.flatten(0, 1)
                loss_emd_raw = torch.mean(
                    self.earth_mover_loss(
                        pred_sv_points_flat.float(),
                        target_obj_pc_sparse_flat.float(),
                        transpose=False,
                    ) / pred_sv_points_flat.shape[1]
                )
                loss_emd_terms.append(loss_emd_raw)
            if (
                master_obj_kp21_rest is not None
                and target_joints_3d_no_rot is not None
                and target_obj_rot6d_gt is not None
                and target_obj_trans_gt is not None
                and self.obj_sym_corner_weight > 0
            ):
                corner_rest_view = master_obj_kp21_rest[:, None, :8, :].expand(-1, pred_obj_view_rot6d_cam.shape[1], -1, -1)
                pred_sv_center_abs = pred_ref_hand_root_views.squeeze(-2) + pred_obj_view_trans
                pred_sv_corners = self._build_object_points_from_pose(
                    corner_rest_view.flatten(0, 1),
                    pred_obj_view_rot6d_cam.flatten(0, 1),
                    pred_sv_center_abs.flatten(0, 1),
                )
                gt_sv_center_abs = target_joints_3d_no_rot[:, :, self.center_idx] + target_obj_trans_gt
                obj_ids_view = obj_ids[:, None].expand(-1, pred_obj_view_rot6d_cam.shape[1]).reshape(-1) if obj_ids is not None else None
                loss_sym_terms.append(
                    self._symmetry_corner_loss(
                        pred_sv_corners,
                        corner_rest_view.flatten(0, 1),
                        target_obj_rot6d_gt.flatten(0, 1),
                        gt_sv_center_abs.flatten(0, 1),
                        obj_ids_view,
                    )
                )
        ...
        loss_obj_emd = zero
        loss_obj_sym = zero
        if loss_chamfer_terms and self.obj_chamfer_weight > 0:
            loss_obj_chamfer = torch.stack(loss_chamfer_terms).mean() * self.obj_chamfer_weight
        if loss_emd_terms and self.obj_emd_weight > 0:
            loss_obj_emd = torch.stack(loss_emd_terms).mean() * self.obj_emd_weight
        if loss_sym_terms and self.obj_sym_corner_weight > 0:
            loss_obj_sym = torch.stack(loss_sym_terms).mean() * self.obj_sym_corner_weight
        return loss_obj_chamfer, loss_obj_emd, loss_obj_sym
    ...
    def _maybe_log_stage_transition_loss(self, mode, epoch_idx, stage_name, loss_dict, pose_metric_dict=None):
        ...
        logger.warning(
            ...
            f"obj_sym={(_to_float('loss_obj_sym_corner') or 0.0):.4f} "
    ...
    def _sv_object_param_to_rotmat(self, rot_param):
        rot_param = self._sanitize_obj_tensor(rot_param, self.sv_obj_axisang_abs_max)
        return aa_to_rotmat(rot_param.reshape(-1, 3)).view(*rot_param.shape[:-1], 3, 3)
    ...
    def _predict_singleview_object_pose(
        ...
        obj_tokens = self._sanitize_obj_tensor(obj_tokens.float(), self.sv_obj_feat_abs_max)
        hand_tokens = self._sanitize_obj_tensor(hand_tokens.float(), self.sv_obj_feat_abs_max)
        obj_query = self.sv_object_token_adapter(obj_tokens)
        hand_kv = self.sv_hand_token_adapter(hand_tokens)
        obj_ref, _ = self.sv_object_hand_cross_attn(
            query=obj_query,
            key=hand_kv,
            value=hand_kv,
            need_weights=False,
        )
        obj_pose_token = self.sv_object_pose_norm(obj_query + obj_ref)
        obj_pose_token = self.sv_object_pose_norm(obj_pose_token + self.sv_object_pose_ffn(obj_pose_token))
        shared_feat = self.sv_object_pose_feat(obj_pose_token.squeeze(1))
        shared_feat = self._sanitize_obj_tensor(shared_feat, self.sv_obj_feat_abs_max)

        pred_rotaxisang_cam = self._sanitize_obj_tensor(
            self.sv_object_rot_regressor(shared_feat),
            self.sv_obj_axisang_abs_max,
        ).view(batch_size, num_cams, 3)
        pred_rotmat_cam = self._sv_object_param_to_rotmat(pred_rotaxisang_cam)
        pred_rot6d_cam = self._sanitize_obj_tensor(
            rotmat_to_rot6d(pred_rotmat_cam.reshape(-1, 3, 3)).view(batch_size, num_cams, 6),
            self.sv_obj_rot6d_abs_max,
        )

        cam_to_master = cam_extr[..., :3, :3]
        master_to_cam = torch.linalg.inv(cam_extr)
        ...
        pred_rotaxisang_master = self._sanitize_obj_tensor(
            rotmat_to_aa(pred_rotmat_master.reshape(-1, 3, 3)).view(batch_size, num_cams, 3),
            self.sv_obj_axisang_abs_max,
        )
        ...
        hand_root_master = ref_hand[:, self.center_idx:self.center_idx + 1]
        obj_center_master = ref_obj
        pred_trans_master = self._sanitize_obj_tensor(
            (obj_center_master - hand_root_master).squeeze(1),
            self.sv_obj_trans_abs_max,
        )
        pred_trans_master_views = pred_trans_master.unsqueeze(1).expand(-1, num_cams, -1)

        hand_root_cam = batch_cam_extr_transf(master_to_cam, hand_root_master.unsqueeze(1).expand(-1, num_cams, -1, -1)).squeeze(-2)
        obj_center_cam = batch_cam_extr_transf(master_to_cam, obj_center_master.unsqueeze(1).expand(-1, num_cams, -1, -1)).squeeze(-2)
        pred_trans_cam = self._sanitize_obj_tensor(obj_center_cam - hand_root_cam, self.sv_obj_trans_abs_max)

        pose_master = torch.cat((pred_rotaxisang_master.detach(), pred_trans_master_views.detach()), dim=-1)
        pose_master = self._sanitize_obj_tensor(pose_master, self.sv_obj_feat_abs_max)
        pose_tokens = self.sv_object_init_pose_token(pose_master.reshape(-1, 6)).view(batch_size, num_cams, -1)
        ...
        pred_rotmat_master = pred_rotmat_master.detach()
        base_rotmat = self._project_matrix_to_rotation(pred_rotmat_master.mean(dim=1))
        base_rotaxisang = self._sanitize_obj_tensor(rotmat_to_aa(base_rotmat), self.sv_obj_axisang_abs_max)
        base_trans = self._sanitize_obj_tensor(pred_trans_master.detach(), self.sv_obj_trans_abs_max)

        init_input = torch.cat((pose_mean, pose_max, base_rotaxisang, base_trans), dim=-1)
        ...
        delta_rot = torch.tanh(init_delta[:, :3]) * 0.5
        delta_trans = torch.tanh(init_delta[:, 3:]) * self.data_preset_cfg.BBOX_3D_SIZE

        fused_rotaxisang = self._sanitize_obj_tensor(base_rotaxisang + delta_rot, self.sv_obj_axisang_abs_max)
        fused_rotmat = self._sv_object_param_to_rotmat(fused_rotaxisang)
        fused_rot6d = self._sanitize_obj_tensor(
            rotmat_to_rot6d(fused_rotmat),
            self.sv_obj_rot6d_abs_max,
        )
        fused_trans = self._sanitize_obj_tensor(base_trans + delta_trans, self.sv_obj_trans_abs_max)

        return {
            "obj_view_rot_axisang_cam": pred_rotaxisang_cam,
            "obj_view_rot_axisang_master": pred_rotaxisang_master,
            "obj_view_rot6d_cam": pred_rot6d_cam,
            "obj_view_rot6d_master": pred_rot6d_master,
            "obj_view_trans": pred_trans_cam,
            "obj_view_trans_master": pred_trans_master_views,
            "obj_fused_rot_axisang_sv": fused_rotaxisang,
            "obj_fused_rot6d_sv": fused_rot6d,
            "obj_fused_trans_sv": fused_trans,
            "obj_init_rot_axisang": fused_rotaxisang,
            "obj_init_rot6d": fused_rot6d,
            "obj_init_trans": fused_trans,
        }

    def _build_object_center_heatmap_targets(self, gt_obj_center_pixel, valid_mask, heatmap_shape, image_shape, device, dtype):
        batch_size, num_views, num_points = gt_obj_center_pixel.shape[:3]
        hm_h, hm_w = heatmap_shape
        img_h, img_w = image_shape
        target = np.zeros((batch_size, num_views, num_points, hm_h, hm_w), dtype=np.float32)
        gt_np = gt_obj_center_pixel.detach().cpu().numpy()
        valid_np = valid_mask.detach().cpu().numpy()
        img_size = np.array([img_w, img_h], dtype=np.float32)
        hm_size = np.array([hm_w, hm_h], dtype=np.float32)
        ...
        for b in range(batch_size):
            for v in range(num_views):
                for p in range(num_points):
                    if valid_np[b, v, p] <= 0:
                        continue
                    coord_hm = ((gt_np[b, v, p] / img_size) * hm_size).astype(np.int32)
                    target[b, v, p], _ = generate_heatmap(target[b, v, p], coord_hm, self.data_preset_cfg.HEATMAP_SIGMA)
    ...
    def _forward_impl(self, batch, **kwargs):
        ...
        hand_tokens_pose = hand_obj_comb_feats[:, :self.num_hand_joints]
        obj_tokens_pose = hand_obj_comb_feats[:, self.num_hand_joints:]
        ...
        ref_hand, ref_obj = self.project(pred_hand_jts, pred_obj_jts, hand_score_ref, obj_score_ref, K, T_c2m, batch_size, num_cams)
        ...
        if interaction_mode in ["hand", "ho"]:
            sv_rot_outputs = self._predict_singleview_object_pose(
                obj_tokens=obj_tokens_pose,
                hand_tokens=hand_tokens_pose,
                cam_extr=batch["target_cam_extr"],
                master_id=batch["master_id"],
                ref_hand=ref_hand_anchor,
                ref_obj=ref_obj_anchor,
            )

        proj_preds = {
            "ref_hand": ref_hand,
            "ref_obj_center": ref_obj,
        }
        # prepare image_metas
        img_metas = {
        ...
        obj_init_rot_axisang = pt_preds.get("obj_init_rot_axisang", None)
        obj_fused_rot_axisang_sv = pt_preds.get("obj_fused_rot_axisang_sv", None)
        obj_view_rot_axisang_cam = pt_preds.get("obj_view_rot_axisang_cam", None)
        obj_view_rot_axisang_master = pt_preds.get("obj_view_rot_axisang_master", None)
        ...
        if sv_rot_outputs is not None:
            obj_init_rot_axisang = sv_rot_outputs.get("obj_init_rot_axisang", obj_init_rot_axisang)
            obj_fused_rot_axisang_sv = sv_rot_outputs.get("obj_fused_rot_axisang_sv", obj_fused_rot_axisang_sv)
            obj_view_rot_axisang_cam = sv_rot_outputs.get("obj_view_rot_axisang_cam", obj_view_rot_axisang_cam)
            obj_view_rot_axisang_master = sv_rot_outputs.get("obj_view_rot_axisang_master", obj_view_rot_axisang_master)
        ...
        final_preds.update({
            ...
            "obj_init_rot_axisang": obj_init_rot_axisang,
            "obj_init_rot6d": obj_init_rot6d,
            "obj_init_trans": obj_init_trans,
            "obj_fused_rot_axisang_sv": obj_fused_rot_axisang_sv,
            "obj_fused_rot6d_sv": obj_fused_rot6d_sv,
            "obj_fused_trans_sv": obj_fused_trans_sv,
            "obj_view_rot_axisang_cam": obj_view_rot_axisang_cam,
            "obj_view_rot_axisang_master": obj_view_rot_axisang_master,
    ...
    def compute_sv_object_pose_metrics(self, preds, gt):
        ...
        obj_points_rest = gt.get("master_obj_sparse_rest", None)
        if obj_points_rest is None:
            return zero_metrics
        ...
        obj_points_rest = torch.nan_to_num(
            obj_points_rest.detach().to(device=pred_obj_view_trans.device, dtype=pred_obj_view_trans.dtype),
            nan=0.0,
            posinf=10.0,
            neginf=-10.0,
        )
        obj_points_surface_rest = gt.get("master_obj_eval_rest", None)
        if obj_points_surface_rest is not None:
            obj_points_surface_rest = torch.nan_to_num(
                obj_points_surface_rest.detach().to(device=pred_obj_view_trans.device, dtype=pred_obj_view_trans.dtype),
                nan=0.0,
                posinf=10.0,
                neginf=-10.0,
            )

        batch_size, num_views = pred_obj_view_rot6d_cam.shape[:2]
        ...
        obj_points_rest = obj_points_rest.reshape(batch_size * num_views, obj_points_rest.shape[-2], 3)

        if obj_points_surface_rest is not None:
            if obj_points_surface_rest.dim() == 3:
                obj_points_surface_rest = obj_points_surface_rest.unsqueeze(1).expand(-1, num_views, -1, -1)
            obj_points_surface_rest = obj_points_surface_rest.reshape(batch_size * num_views, obj_points_surface_rest.shape[-2], 3)

        pred_rot_flat = pred_obj_view_rot6d_cam.reshape(batch_size * num_views, 6)
        pred_trans_flat = pred_obj_view_trans.reshape(batch_size * num_views, 3)
        gt_rot_flat = target_obj_rot6d_gt.reshape(batch_size * num_views, 6)
        gt_trans_flat = target_obj_trans_gt.reshape(batch_size * num_views, 3)

        pred_points = self.OBJ_POSE_VAL._apply_pose(obj_points_rest, pred_rot_flat, pred_trans_flat)
        gt_points = self.OBJ_POSE_VAL._apply_pose(obj_points_rest, gt_rot_flat, gt_trans_flat)
        add = torch.norm(pred_points - gt_points, dim=-1).mean(dim=-1).view(batch_size, num_views)
        adds = torch.cdist(pred_points.float(), gt_points.float()).min(dim=-1)[0].mean(dim=-1).view(batch_size, num_views)

        obj_ids = gt.get("obj_id", None)
        if obj_ids is not None:
            if not torch.is_tensor(obj_ids):
                obj_ids = torch.as_tensor(obj_ids, device=pred_obj_view_trans.device)
            obj_ids = obj_ids.to(device=pred_obj_view_trans.device, dtype=torch.long)
            if obj_ids.dim() == 1:
                obj_ids = obj_ids.unsqueeze(1).expand(-1, num_views)
            else:
                while obj_ids.dim() > 2:
                    obj_ids = obj_ids[..., 0]
                if obj_ids.shape[1] == 1 and num_views > 1:
                    obj_ids = obj_ids.expand(-1, num_views)
            obj_ids = obj_ids.reshape(batch_size * num_views)

        mssd = self.OBJ_POSE_VAL._compute_mssd(
            pred_rot6d=pred_rot_flat,
            pred_trans=pred_trans_flat,
            gt_rot6d=gt_rot_flat,
            gt_trans=gt_trans_flat,
            obj_points_surface_rest=obj_points_surface_rest,
            obj_ids=obj_ids,
        )
        mssd_mean = zero if mssd is None else mssd.mean()
        ...
        return {
            ...
            "metric_sv_obj_add": (add * valid_weight).sum() / denom,
            "metric_sv_obj_adds": (adds * valid_weight).sum() / denom,
            "metric_sv_obj_mssd": mssd_mean,
        }
    ...
    def compute_loss(self, preds, gt, stage_name="stage2", epoch_idx=None, **kwargs):
        ...
        pred_ref_hand_views = batch_cam_extr_transf(
            gt_T_c2m,
            preds["ref_hand"].unsqueeze(1).expand(-1, n_views, -1, -1),
        )
        pred_ref_hand_root_views = pred_ref_hand_views[:, :, self.center_idx:self.center_idx + 1]
        target_obj_rot6d_gt = gt.get("target_rot6d_label", None)
        target_obj_trans_gt = gt.get("target_t_label_rel", None)
        target_obj_kp21_gt = gt.get("target_obj_kp21", None)
        master_obj_kp21_gt = gt.get("master_obj_kp21", None)
        target_joints_3d_no_rot = gt.get("target_joints_3d_no_rot", None)
        ...
        pred_obj_heatmap = pred_uv_heatmap[:, self.num_hand_joints:self.num_hand_joints + self.num_obj_joints]
        ...
        if target_obj_kp21_gt is not None:
            gt_obj_center_3d = target_obj_kp21_gt[:, :, -self.num_obj_joints:].to(device=pred_hand_heatmap.device, dtype=pred_hand_heatmap.dtype)
            gt_obj_center_pixel = batch_cam_intr_projection(
                gt["target_cam_intr"].to(device=pred_hand_heatmap.device, dtype=pred_hand_heatmap.dtype),
                gt_obj_center_3d,
            )
            pred_obj_pixel = preds["pred_obj_pixel"].view(batch_size, n_views, self.num_obj_joints, 2)
            obj_center_valid = torch.isfinite(gt_obj_center_pixel).all(dim=-1) & (gt_obj_center_3d[..., 2] > 1e-6)
            gt_obj_center_pixel = torch.nan_to_num(gt_obj_center_pixel, nan=0.0, posinf=img_scale, neginf=-img_scale)
            obj_heatmap_diff = (pred_obj_pixel - gt_obj_center_pixel) / img_scale
            obj_heatmap_diff = torch.sum(torch.pow(obj_heatmap_diff, 2), dim=3)
            obj_valid_mask = obj_center_valid.to(dtype=obj_heatmap_diff.dtype)
            loss_heatmap_obj = (obj_heatmap_diff * obj_valid_mask).sum() / (obj_valid_mask.sum() + 1e-9)

            gt_obj_center_heatmap = self._build_object_center_heatmap_targets(
                gt_obj_center_pixel=gt_obj_center_pixel,
                valid_mask=obj_center_valid,
                heatmap_shape=pred_obj_heatmap.shape[-2:],
                image_shape=(H, W),
                device=pred_obj_heatmap.device,
                dtype=pred_obj_heatmap.dtype,
            ).flatten(0, 1)
            loss_heatmap_obj_map = self._masked_heatmap_mse(
                pred_obj_heatmap,
                gt_obj_center_heatmap,
                obj_center_valid.flatten(0, 1),
            )
        ...
        loss_heatmap_obj = loss_heatmap_obj * self.heatmap_obj_weight
        loss_heatmap_obj_map = loss_heatmap_obj_map * self.heatmap_obj_map_weight
        ...
        loss_triang_obj = pred_mano_3d_mesh_sv.new_tensor(0.0)
        ...
        if stage_name == "stage1":
            loss_obj_chamfer_stage1, loss_obj_emd_stage1, loss_obj_sym_corner_stage1 = self._compute_stage1_object_geometry_losses(
                preds=preds,
                gt=gt,
                pred_ref_hand_root_views=pred_ref_hand_root_views,
                master_hand_joints_gt=master_hand_joints_gt,
                master_obj_sparse_gt=master_obj_sparse_gt,
                master_obj_rot6d_gt=master_obj_rot6d_gt,
                master_obj_trans_gt=master_obj_trans_gt,
                target_obj_rot6d_gt=target_obj_rot6d_gt,
                target_obj_trans_gt=target_obj_trans_gt,
            )
        else:
            loss_obj_chamfer_stage1 = pred_mano_3d_mesh_sv.new_tensor(0.0)
            loss_obj_emd_stage1 = pred_mano_3d_mesh_sv.new_tensor(0.0)
            loss_obj_sym_corner_stage1 = pred_mano_3d_mesh_sv.new_tensor(0.0)
    ...
    def validation_step(self, batch, step_idx, **kwargs):
        ...
        if stage_name == "stage2":
            self.OBJ_POSE_VAL.feed(
                ...
                obj_points_surface_rest=batch.get("master_obj_eval_rest", None),
                obj_ids=batch.get("obj_id", None),
            )
            ...
            self.summary.add_scalar("OBJ_MSSD_val", obj_measures["Obj_mssd"], step_idx)
    ...
    def format_metric(self, mode="train"):
        ...
        if mode == "train":
            ...
            l_obj_geom = _get_loss("loss_obj_chamfer_stage1") + _get_loss("loss_obj_emd_stage1") + _get_loss("loss_obj_sym_corner_stage1")
            ...
            m_sv_obj_add = _get_loss('metric_sv_obj_add')
            m_sv_obj_adds = _get_loss('metric_sv_obj_adds')
            m_sv_obj_mssd = _get_loss('metric_sv_obj_mssd')
            ...
            if self.current_stage_name == "stage1":
                ...
                return (f"{stage_short}{warmup_suffix} | "
                        f"L {l_total:.3f} | "
                        f"B H/J/P {l_hm:.3f}/{l_3d_jts:.3f}/{l_2d_proj:.3f} | "
                        f"OG C/E/S { _get_loss('loss_obj_chamfer_stage1'):.3f}/{ _get_loss('loss_obj_emd_stage1'):.3f}/{ _get_loss('loss_obj_sym_corner_stage1'):.3f} | "
                        f"KP {_fmt_mm_pair(kp_j, kp_v)} | "
                        f"SV A/S/M {_fmt_mm_pair(m_sv_obj_add, m_sv_obj_adds)}/{_fmt_mm_value(m_sv_obj_mssd)} | "
                        f"SVRT {m_sv_obj_rot_deg:.1f}/{_fmt_mm_value(m_sv_obj_trans)} | "
            ...
            sv_obj_add = _get_loss('metric_sv_obj_add')
            sv_obj_adds = _get_loss('metric_sv_obj_adds')
            sv_obj_mssd = _get_loss('metric_sv_obj_mssd')
            l_obj_geom = _get_loss("loss_obj_chamfer_stage1") + _get_loss("loss_obj_emd_stage1") + _get_loss("loss_obj_sym_corner_stage1")
            l_obj_master_aux = _get_loss("loss_obj_rot") + _get_loss("loss_obj_trans")
            l_obj_master_points = _get_loss("loss_obj_points")
            ...
            if mode == "val_full":
                msg = " | ".join([
                    ...
                    (
                        ...
                        f"MSSD {_fmt_mm_value(self.OBJ_POSE_VAL.mssd.avg)} "
                        ...
                        "Object ADD/ADD-S/MSSD/TranslationEPE/RotationError -/-/-/-/-"
                    ),
                    (
                        f"SVObject ADD/ADD-S {_fmt_mm_pair(sv_obj_add, sv_obj_adds)} "
                        f"MSSD {_fmt_mm_value(sv_obj_mssd)} "
                        f"Rotation/Trans {sv_obj_rot_deg:.1f} deg/{_fmt_mm_value(sv_obj_trans_epe)}"
                        if self.current_stage_name in ["stage1", "stage2"] else
                        "SVObject ADD/ADD-S/MSSD/Rotation/Trans -/-/-/-/-"
                    ),
                    f"SVGeom {l_obj_geom:.3f}",
                    (
            ...
            msg = " | ".join([
                ...
                (
                    f"Obj A/S/M {_fmt_mm_pair(self.OBJ_POSE_VAL.add.avg, self.OBJ_POSE_VAL.adds.avg)}/{_fmt_mm_value(self.OBJ_POSE_VAL.mssd.avg)}"
                    if self.current_stage_name == "stage2" and self.OBJ_POSE_VAL.count > 0 else
                    "Obj A/S/M -/-/-"
                ),
                (
                ...
                (
                    f"SV A/S/M {_fmt_mm_pair(sv_obj_add, sv_obj_adds)}/{_fmt_mm_value(sv_obj_mssd)}"
                    if self.current_stage_name in ["stage1", "stage2"] else
                    "SV A/S/M -/-/-"
                ),
                (
