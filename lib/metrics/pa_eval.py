...
class PAEval(Metric):
    ...
    def feed(self, pred_joints_3d_abs, joints_3d_abs, pred_verts_3d_abs=None, verts_3d_abs=None, **kwargs):
        ...
        joints_3d_abs = joints_3d_abs.detach().cpu().numpy()
        ...
        if self.mesh_score is True:
            ...
            verts_3d_abs = verts_3d_abs.detach().cpu().numpy()
    ...
    def __str__(self) -> str:
        score = f"PA-J {self.pa_mpjpe.avg * 1000.0:6.2f}mm | MP {self.mpjpe.avg:6.4f}"
        if self.mesh_score:
            score += f" | PA-V {self.pa_mpvpe.avg * 1000.0:6.2f}mm | MV {self.mpvpe.avg:6.4f}"
